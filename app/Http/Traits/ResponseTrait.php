...
trait ResponseTrait
{
    ...
    function sendListResponse($message,$data,$qry_params = "",$code = 200, $status = true){
        return response()->json([
            'status'  => $status,
            'message' => $message,
            'results' => [
                'count'   => $data['total'],
                'next_page_url' => ($data['next_page_url'] != null) ? $data['next_page_url']."&".$qry_params : null,
                'prev_page_url' => ($data['prev_page_url'] != null) ? $data['prev_page_url']."&".$qry_params : null,
                'data'    => $data['data'],
            ]
        ],$code);
    }
